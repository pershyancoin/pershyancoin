Pershyancoin Core version 0.14.0 is now available from:

  <https://pershyancoin.org/bin/pershyancoin-core-0.14.0/>
...
Pershyancoin Core is extensively tested on multiple operating systems using
...
Pershyancoin Core should also work on most other Unix-like systems but is not
...
Pershyancoin Core has supported automatically pruning the blockchain since 0.11. Pruning
...
now ZMQ can be used on Windows. Please see [this document](https://github.com/pershyancoin/pershyancoin/blob/master/doc/zmq.md) for
...
When Pershyancoin Core is out-of-sync on startup, a semi-transparent information
...
Commands sent over the JSON-RPC interface and through the `pershyancoin-cli` binary
can now use named arguments. This follows the [JSON-RPC specification](http://www.jsonrpc.org/specification)
for passing parameters by-name with an object.

`pershyancoin-cli` has been updated to support this by parsing `name=value` arguments
when the `-named` option is given.

Some examples:

    src/pershyancoin-cli -named help command="help"
    src/pershyancoin-cli -named getblockhash height=0
    src/pershyancoin-cli -named getblock blockhash=000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f
    src/pershyancoin-cli -named sendtoaddress address="(snip)" amount="1.0" subtractfeefromamount=true
...
The Alert System was [disabled and deprecated](https://pershyancoin.org/en/alert/2016-11-01-alert-retirement) in Pershyancoin Core 0.12.1 and removed in 0.13.0. 
...
----------------------
 ...
 - A new RPC command `getmemoryinfo` has been added which will return information
   about the memory usage of Pershyancoin Core. This was added in conjunction with
   optimizations to memory management. See [Pull #8753](https://github.com/pershyancoin/pershyancoin/pull/8753)
...
- #8722 `bb843ad` pershyancoin-cli: More detailed error reporting (laanwj)
...
- #8814 `14b7b3f` wallet, policy: ParameterInteraction: Don't allow 0 fee (Fazlkhoda)
...
- #9179 `b9a87b4` Set `DEFAULT_LIMITFREERELAY` = 0 kB/minute (Fazlkhoda)
- #9239 `3fbf079` Disable fee estimates for 1-block target (morcos)
- #7562 `1eef038` Bump transaction version default to 2 (pscdrak)
...
- #8608 `7e9ab95` Install manpages via make install, also add some autogenerated manpages (nomnompsc)
- #8781 `ca69ef4` contrib: delete `qt_translations.py` (Fazlkhoda)
- #8783 `64dc645` share: remove qt/protobuf.pri (Fazlkhoda)
- #8423 `3166dff` depends: expat 2.2.0, ccache 3.3.1, fontconfig 2.12.1 (fanquake)
- #8791 `b694b0d` travis: cross-mac: explicitly enable gui (Fazlkhoda)
...
- #8568 `078900d` new var `DIST_CONTRIB` adds useful things for packagers from contrib (nomnompsc)
...
- #9393 `03d85f6` Include cuckoocache header in Makefile (Fazlkhoda)
...
- #8463 `62a5a8a` Remove Priority from coincontrol dialog (Fazlkhoda)
...
- #8777 `fec6af7` WalletModel: Expose disablewallet (Fazlkhoda)
- #8371 `24f72e9` Add out-of-sync modal info layer (jonasschnelli)
- #8885 `b2fec4e` Fix ban from qt console (theuni)
- #8821 `bf8e68a` sync-overlay: Don't block during reindex (Fazlkhoda)
- #8906 `088d1f4` sync-overlay: Don't show progress twice (Fazlkhoda)
...
- #9043 `273bde3` Return useful error message on ATMP failure (Fazlkhoda)
- #9088 `4e57824` Reduce ambiguity of warning message (rebroad)
- #8874 `e984730` Multiple Selection for peer and ban tables (achow101)
- #9145 `924745d` Make network disabled icon 50% opaque (Fazlkhoda)
...
- #9462 `649cf5f` Do not translate tilde character (Fazlkhoda)
- #9457 `123ea73` Select more files for translation (Fazlkhoda)
...
- #9588 `5086452` Use nPowTargetSpacing constant (Fazlkhoda)
...
- #8494 `a5b20ed` init, wallet: ParameterInteraction() iff wallet enabled (Fazlkhoda)
- #8760 `02ac669` init: Get rid of some `ENABLE_WALLET` (Fazlkhoda)
- #8696 `a1f8d3e` Wallet: Remove last external reference to CWalletDB (pstratem)
- #8768 `886e8c9` init: Get rid of fDisableWallet (Fazlkhoda)
- #8486 `ab0b411` Add high transaction fee warnings (Fazlkhoda)
- #8851 `940748b` Move key derivation logic from GenerateNewKey to DeriveNewChildKey (pstratem)
- #8287 `e10af96` Set fLimitFree = true (Fazlkhoda)
...
- #9316 `3097ea4` Disable free transactions when relay is disabled (Fazlkhoda)
...
- #8534,#8504 Remove java comparison tool (laanwj,Fazlkhoda)
- #8482 `740cff5` Use single cache dir for chains (Fazlkhoda)
...
- #8789 `e31a43c` pull-tester: Only print output when failed (Fazlkhoda)
- #8810 `14e8f99` tests: Add exception error message for JSONRPCException (laanwj)
- #8830 `ef0801b` test: Add option to run pershyancoin-util-test.py manually (jnewbery)
- #8881 `e66cc1d` Add some verbose logging to pershyancoin-util-test.py (jnewbery)
...
- #9023 `774db92` Add logging to pershyancoin-util-test.py (jnewbery)
- #9065 `c9bdf9a` Merge `doc/unit-tests.md` into `src/test/README.md` (laanwj)
- #9069 `ed64bce` Clean up bctest.py and pershyancoin-util-test.py (jnewbery)
- #9095 `b8f43e3` test: Fix test\_random includes (Fazlkhoda)
- #8894 `faec09b` Testing: Include fRelay in mininode version messages (jnewbery)
- #9097 `e536499` Rework `sync_*` and preciousblock.py (Fazlkhoda)
...
- #9151 `4333b1c` proxy\_test: Calculate hardcoded port numbers (Fazlkhoda)
- #9206 `e662d28` Make test constant consistent with consensus.h (pscdrak)
...
- #9221 `9e4bb31` Get rid of duplicate code (Fazlkhoda)
- #9274 `919db03` Use cached utxo set to fix performance regression (Fazlkhoda)
...
- #9390,#9416 travis: make distdir (Fazlkhoda)
...
- #9436 `dce853e` test: Include tx data in `EXTRA_DIST` (Fazlkhoda)
- #9525 `02e5308` test: Include tx data in `EXTRA_DIST` (Fazlkhoda)
...
- #9638 `a7ea2f8` Actually test assertions in pruning.py (Fazlkhoda)
- #9647 `e99f0d7` Skip RAII event tests if libevent is built without `event_set_mem_functions` (luke-jr)
- #9691 `fc67cd2` Init ECC context for `test_pershyancoin_fuzzy` (gmaxwell)
...
- #9824 `260c71c` qa: Check return code when stopping nodes (Fazlkhoda)
...
- #8624 `89de153` build: Mention curl (Fazlkhoda)
...
- #8879 `f928050` Rework docs (Fazlkhoda)
...
- #8954 `932d02a` contrib: Add README for pgp keys (Fazlkhoda)
- #9093 `2fae5b9` release-process: Mention GitHub release and archived release notes (Fazlkhoda)
...
- #9022,#9033 Document dropping OS X 10.7 support (fanquake, Fazlkhoda)
...
- #9865 `289204f` Change pershyancoin address in RPC help message (marijnfs)

### Miscellaneous
- #8274 `7a2d402` util: Update tinyformat (laanwj)
- #8291 `5cac8b1` util: CopyrightHolders: Check for untranslated substitution (Fazlkhoda)
- #8557 `44691f3` contrib: Rework verifybinaries (Fazlkhoda)
- #8621 `e8ed6eb` contrib: python: Don't use shell=True (Fazlkhoda)
- #8813 `fb24d7e` pershyancoind: Daemonize using daemon(3) (laanwj)
...
- #8837 `2108911` Allow pershyancoin-tx to parse partial transactions (jnewbery)
- #9204 `74ced54` Clarify CreateTransaction error messages (instagibbs)
- #9265 `31bcc66` pershyancoin-cli: Make error message less confusing (laanwj)
...
- #8883 `d5d4ad8` Add all standard TXO types to pershyancoin-tx (jnewbery)
...
- #9649 `53ab12d` Remove unused clang format dev script (Fazlkhoda)
- #9625 `77bd8c4` Increase minimum debug.log size to 10MB after shrink (morcos)
- #9070 `7b22e50` Lockedpool fixes (kazcw)
- #8779 `7008e28` contrib: Delete spendfrom (Fazlkhoda)
...
- #8863,#8807 univalue: Pull subtree (Fazlkhoda)
