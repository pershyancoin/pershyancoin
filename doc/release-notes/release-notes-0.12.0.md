Pershyancoin Core version 0.12.0 is now available from:

  <https://pershyancoin.org/bin/pershyancoin-core-0.12.0/>
...
installer (on Windows) or just copy over /Applications/Pershyancoin-Qt (on Mac) or
pershyancoind/pershyancoin-qt (on Linux).
...
backwards-compatible with pre-0.10 versions of Pershyancoin Core or other software:
...
pre-0.12 versions of Pershyancoin Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start Pershyancoin Core version 0.11 or
earlier.
...
ECDSA signatures inside Pershyancoin transactions now use validation using
[libsecp256k1](https://github.com/pershyancoin-core/secp256k1) instead of OpenSSL.
...
(https://github.com/pershyancoin/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of Pershyancoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Pershyancoin Core was to raise the default minimum
relay fee.

Pershyancoin Core 0.12 will have a strict maximum size on the mempool. The
...
Pershyancoin Core 0.12 also introduces new default policy limits on the length and
...
Pershyancoin Core 0.12 nodes. Pershyancoin Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/pershyancoin/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in Pershyancoin Core 0.12 does not yet have support for
...
This allows running pershyancoind without having to do any manual configuration.
...
Pershyancoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Pershyancoin Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In Pershyancoin Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in Pershyancoin Core 0.13, and it is at this time undecided whether the
...
Pershyancoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Pershyancoin Core automatically creates a hidden service to listen on, without
manual configuration. Pershyancoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Pershyancoin Core is listening, and
...
Pershyancoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Pershyancoin
Core to use floating fees. By default, Pershyancoin Core will use floating
fees.
...
At all times, Pershyancoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) PSC.
Furthermore, Pershyancoin Core will never create transactions paying less than
...
`pershyancoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of v0.11.0](https://github.com/pershyancoin/pershyancoin/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/pershyancoin/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/pershyancoin/bips/blob/master/bip-0065.mediawiki)
...
caching. A sample config for apache2 could look like:
    ...
    <Location /pershyancoinrpc>
        ...
        # optional bypass pershyancoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< pershyancoinrpc:<password>
    </Location>
...
Restarting pershyancoind will no longer clear out the list of banned peers; instead
...
- #6647 `a3babc8` Sanitize uacomment (Fazlkhoda)
...
- #7290 `b16b5bc` Add missing options help (Fazlkhoda)
...
- #6214 `87406aa` [OSX] revert renaming of Pershyancoin-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new pershyancoin-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include pershyancoin-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #7103 `a775182` [wallet, rpc tests] Fix settxfee, paytxfee (Fazlkhoda)
...
- #7293 `ff9b610` Add regression test for vValue sort order (Fazlkhoda)
- #7306 `4707797` Make sure conflicted wallet tx's update balances (Alex Morcos)
- #7381 `621bbd8` [walletdb] Fix syntax error in key parser (Fazlkhoda)
...
- #6505 `100c9d3` cleanup icons (Fazlkhoda)
...
- #6653 `e04b2fa` Pop debug window in foreground when opened twice (Fazlkhoda)
- #6864 `c702521` Use monospace font (Fazlkhoda)
- #6887 `3694b74` Update coin control and smartfee labels (Fazlkhoda)
- #7000 `814697c` add shortcurts for debug-/console-window (Jonas Schnelli)
- #6951 `03403d8` Use maxTxFee instead of 10000000 (Fazlkhoda)
...
- #7319 `1320300` Intro: Display required space (Fazlkhoda)
...
- #7364 `7726c48` [qt] Windows: Make rpcconsole monospace font larger (Fazlkhoda)
- #7384 `294f432` [qt] Peertable: Increase SUBVERSION_COLUMN_WIDTH (Fazlkhoda)
...
- #6390 `6a73d66` tests: Fix pershyancoin-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of pershyancoin-cli on secp256k1 (Wladimir J. van der Laan)
...
- #6827 `87e5539` [rpc-tests] Check return code (Fazlkhoda)
...
- #6822 `9fa54a1` [tests] Be more strict checking dust (Fazlkhoda)
...
- #7209 `f3ad812` test: don't override PERSHYANCOIND and PERSHYANCOINCLI if they're set (Wladimir J. van der Laan)
...
- #7229 `1ed938b` [qa] wallet: Check if maintenance changes the balance (Fazlkhoda)
...
- #6387 `11576a5` [pershyancoin-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add PERSHYANCOIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libpershyancoinconsensus (Braydon Fuller)
...
- #6504 `b6fee6b` Rationalize currency unit to "PSC" (Ross Nicoll)
...
- #6713 `f6ce59c` SanitizeString: Allow hypen char (Fazlkhoda)
...
- #6790 `fa1d252` devtools: add clang-format.py (Fazlkhoda)
