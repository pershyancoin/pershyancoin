Pershyancoin Core version 0.13.0 is now available from:

  <https://pershyancoin.org/bin/pershyancoin-core-0.13.0/>
...
updates will be released anymore. Without security updates, using a pershyancoin
wallet on a XP machine is irresponsible at least.

In addition to that, with 0.12.x there have been varied reports of Pershyancoin Core
randomly crashing on Windows XP. It is [not clear](https://github.com/pershyancoin/pershyancoin/issues/7681#issuecomment-217439891)
...
- Adding `dbcache=100` in pershyancoin.conf
...
pershyancoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/pershyancoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Pershyancoin Core code base has
...
- `pershyancoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `pershyancoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
...
compatible peers.
[BIP 152](https://github.com/pershyancoin/bips/blob/master/bip-0152.mediawiki)
...
HD wallets are incompatible with older versions of Pershyancoin Core.

[Pull request](https://github.com/pershyancoin/pershyancoin/pull/8035/files), [BIP 32](https://github.com/pershyancoin/bips/blob/master/bip-0032.mediawiki)
...
141](https://github.com/pershyancoin/bips/blob/master/bip-0141.mediawiki), [BIP
143](https://github.com/pershyancoin/bips/blob/master/bip-0143.mediawiki), [BIP
144](https://github.com/pershyancoin/bips/blob/master/bip-0144.mediawiki), and [BIP
145](https://github.com/pershyancoin/bips/blob/master/bip-0145.mediawiki) are
finished and included in this release.  However, BIP 141 does not yet specify
activation parameters on mainnet, and so this release does not support segwit
use on mainnet.  Testnet use is supported, and after BIP 141 is updated with
proposed parameters, a future release of Pershyancoin Core is expected that
...
Witness)] (https://github.com/pershyancoin/bips/blob/master/bip-0141.mediawiki).
...
(https://github.com/pershyancoin/bips/blob/master/bip-0141.mediawiki) for additional
details.)
...
The two phases are distinct in the Pershyancoin-Qt GUI. During the first one,
...
- The optional new p2p message "feefilter" is implemented and the protocol
  ...
  feerate. [BIP 133](https://github.com/pershyancoin/bips/blob/master/bip-0133.mediawiki)
...
65](https://github.com/pershyancoin/bips/blob/master/bip-0065.mediawiki)

  - OP_NOP3 has been renamed to OP_CHECKSEQUENCEVERIFY by [BIP 
112](https://github.com/pershyancoin/bips/blob/master/bip-0112.mediawiki)
...
- Each ZMQ notification now contains an up-counting sequence number that allows
  ...
  PR [#7762](https://github.com/pershyancoin/pershyancoin/pull/7762).
...
- #7550 `8b958ab` Input-from-stdin mode for pershyancoin-cli (laanwj)
...
- #8153 `75ec320` `fundrawtransaction` feeRate: Use PSC/kB (Fazlkhoda)
...
- #8317 `304eff3` Don't use floating point in rpcwallet (Fazlkhoda)
...
- #7084 `11d74f6` Replace maxFeeRate of 10000*minRelayTxFee with maxTxFee in mempool (Fazlkhoda)
...
- #7692 `29b2be6` Remove P2P alert system (pscdrak)
...
- #8275 `042c323` Remove bad chain alert partition check (pscdrak)
...
- #7619 `18b3f1b` Add missing sudo entry in gitian VM setup (pscdrak)
- #7616 `639ec58`  [depends] Delete unused patches  (Fazlkhoda)
- #7658 `c15eb28` Add curl to Gitian setup instructions (pscdrak)
...
- #7283 `fd9881a` [gitian] Default reference_datetime to commit author date (Fazlkhoda)
...
- #8194 `cca1c8c` [gitian] set correct PATH for wrappers (Fazlkhoda)
...
- #8304 `0cca2fe` [travis] Update SDK_URL (Fazlkhoda)
...
- #8358 `cfd1280` Gbuild: Set memory explicitly (default is too low) (Fazlkhoda)
...
- #7218 `a1c185b` Fix misleading translation (Fazlkhoda)
- #7214 `be9a9a3` qt5: Use the fixed font the system recommends (Fazlkhoda)
...
- #7334 `e1060c5` coincontrol workaround is still needed in qt5.4 (fixed in qt5.5) (Fazlkhoda)
...
- #7613 `3798e5d` Add autocomplete to pershyancoin-qt's console window (GamerSg)
...
- #7686 `f034bce` Remove 0-fee from send dialog (Fazlkhoda)
...
- #7636 `fb0ac48` Add pershyancoin address label to request payment QR code (makevoid)
- #8231 `760a6c7` Fix a bug where the SplashScreen will not be hidden during startup (jonasschnelli)
- #8256 `af2421c` BUG: pershyancoin-qt crash (fsb4000)
- #8257 `ff03c50` Do not ask a UI question from pershyancoind (sipa)
- #8288 `91abb77` Network-specific example address (laanwj)
- #7707 `a914968` UI support for abandoned transactions (jonasschnelli)
- #8207 `f7a403b` Add a link to the Pershyancoin-Core repository and website to the About Dialog (Fazlkhoda)
...
- #7608 `0735c0c` Move hardcoded file name out of log messages (Fazlkhoda)
...
- #7320 `d3dfc6d` Test walletpassphrase timeout (Fazlkhoda)
- #7208 `47c5ed1` Make max tip age an option instead of chainparam (laanwj)
- #7372 `21376af` Trivial: [qa] wallet: Print maintenance (Fazlkhoda)
- #7280 `668906f` [travis] Fail when documentation is outdated (Fazlkhoda)
- #7177 `93b0576` [qa] Change default block priority size to 0 (Fazlkhoda)
...
- #7531 `18cb2d5` Add bip68-sequence.py to extended rpc tests (pscdrak)
- #7536 `ce5fc02` test: test leading spaces for ParseHex (laanwj)
- #7620 `1b68de3` [travis] Only run check-doc.py once (Fazlkhoda)
- #7455 `7f96671` [travis] Exit early when check-doc.py fails (Fazlkhoda)
- #7667 `56d2c4e` Move GetTempPath() to testutil (musalbas)
- #7517 `f1ca891` test: script_error checking in script_invalid tests (laanwj)
- #7684 `3d0dfdb` Extend tests (Fazlkhoda)
...
- #7702 `29e1131` Add tests verifychain, lockunspent, getbalance, listsinceblock (Fazlkhoda)
- #7720 `3b4324b` rpc-test: Normalize assert() (Fazlkhoda)
- #7757 `26794d4` wallet: Wait for reindex to catch up (Fazlkhoda)
- #7764 `a65b36c` Don't run pruning.py twice (Fazlkhoda)
- #7773 `7c80e72` Fix comments in tests (pscdrak)
- #7489 `e9723cb` tests: Make proxy_test work on travis servers without IPv6 (laanwj)
- #7801 `70ac71b` Remove misleading "errorString syntax" (Fazlkhoda)
- #7803 `401c65c` maxblocksinflight: Actually enable test (Fazlkhoda)
- #7802 `3bc71e1` httpbasics: Actually test second connection (Fazlkhoda)
- #7849 `ab8586e` tests: add varints_bitpatterns test (laanwj)
- #7846 `491171f` Clean up lockorder data of destroyed mutexes (sipa)
- #7853 `6ef5e00` py2: Unfiddle strings into bytes explicitly (Fazlkhoda)
- #7878 `53adc83` [test] bctest.py: Revert faa41ee (Fazlkhoda)
- #7798 `cabba24` [travis] Print the commit which was evaluated (Fazlkhoda)
- #7833 `b1bf511` tests: Check Content-Type header returned from RPC server (laanwj)
- #7851 `fa9d86f` pull-tester: Don't mute zmq ImportError (Fazlkhoda)
...
- #7980 `88b77c7` Smartfees: Properly use ordered dict (Fazlkhoda)
- #7814 `77b637f` Switch to py3 (Fazlkhoda)
...
- #7972 `423ca30` pull-tester: Run rpc test in parallel  (Fazlkhoda)
- #8039 `69b3a6d` Bench: Add crypto hash benchmarks (laanwj)
- #8041 `5b736dd` Fix bip9-softforks blockstore issue (Fazlkhoda)
- #7994 `1f01443` Add op csv tests to script_tests.json (Christewart)
- #8038 `e2bf830` Various minor fixes (Fazlkhoda)
- #8072 `1b87e5b` Travis: 'make check' in parallel and verbose (theuni)
- #8056 `8844ef1` Remove hardcoded "4 nodes" from test_framework (Fazlkhoda)
- #8047 `37f9a1f` Test_framework: Set wait-timeout for pershyancoind procs (Fazlkhoda)
- #8095 `6700cc9` Test framework: only cleanup on successful test runs (sdaftuar)
- #8098 `06bd4f6` Test_framework: Append portseed to tmpdir (Fazlkhoda)
...
- #8164 `0f24eaf` [Pershyancoin-Tx] fix missing test fixtures, fix 32bit atoi issue (jonasschnelli)
...
- #8201 `36b7400` fundrawtransaction: Fix race, assert amounts (Fazlkhoda)
- #8214 `ed2cd59` Mininode: fail on send_message instead of silent return (Fazlkhoda)
- #8215 `a072d1a` Don't use floating point in wallet tests (Fazlkhoda)
- #8066 `65c2058` Test_framework: Use different rpc_auth_pair for each node (Fazlkhoda)
- #8216 `0d41d70` Assert 'changePosition out of bounds'  (Fazlkhoda)
...
- #7744 `d8e862a` test_framework: detect failure of pershyancoind startup (laanwj)
- #8280 `115735d` Increase sync_blocks() timeouts in pruning.py (Fazlkhoda)
- #8340 `af9b7a9` Solve trivial merge conflict in p2p-segwit.py (Fazlkhoda)
- #8067 `3e4cf8f` Travis: use slim generic image, and some fixups (theuni)
- #7951 `5c7df70` Test_framework: Properly print exception (Fazlkhoda)
- #8070 `7771aa5` Remove non-determinism which is breaking net_tests #8069 (EthanHeilman)
- #8309 `bb2646a` Add wallet-hd test (Fazlkhoda)
...
- #7796 `536b75e` Add support for negative fee rates, fixes `prioritizetransaction` (Fazlkhoda)
...
- #7855 `b778e59` Replace precise with trusty (Fazlkhoda)
- #7975 `fc23fee` Update pershyancoin-core GitHub links (Fazlkhoda)
- #8034 `e3a8207` Add basic git squash workflow (fanquake)
- #7813 `214ec0b` Update port in tor.md (Fazlkhoda)
...
- #7290 `c851d8d` [init] Add missing help for args (Fazlkhoda)
...
- #7540 `ff46dd4` Rename OP_NOP3 to OP_CHECKSEQUENCEVERIFY (pscdrak)
- #8289 `26316ff` bash-completion: Adapt for 0.12 and 0.13 (roques)
- #7453 `3dc3149` Missing patches from 0.12 (Fazlkhoda)
...
- #7304 `b8f485c` [contrib] Add clang-format-diff.py (Fazlkhoda)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/pershyancoin/).
