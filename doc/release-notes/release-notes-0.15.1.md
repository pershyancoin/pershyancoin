Pershyancoin Core version *0.15.1* is now available from:

  <https://pershyancoincore.org/bin/pershyancoin-core-0.15.1/>

or

  <https://pershyancoin.org/bin/pershyancoin-core-0.15.1/>
...
installer (on Windows) or just copy over `/Applications/Pershyancoin-Qt` (on Mac)
or `pershyancoind`/`pershyancoin-qt` (on Linux).
...
A number of changes to the way Pershyancoin Core deals with peer connections and invalid blocks
...
- #11210 `f3f7891` Stop test_pershyancoin-qt touching ~/.pershyancoin (MeshCollider)
...
### Miscellaneous
- #11377 `75997c3` Disallow uncompressed pubkeys in pershyancoin-tx [multisig] output adds (TheBlueMatt)
