    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return PershyancoinUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, PershyancoinUnits::separatorNever);
            else
                return PershyancoinUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != nullptr) ? tr("Requested") + " ("+PershyancoinUnits::name(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
