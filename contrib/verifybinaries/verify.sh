...
WORKINGDIR="/tmp/pershyancoin_verify_binaries"
TMPFILE="hashes.tmp"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'pershyancoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Pershyancoin Core binary release signing key installed?"
   fi
