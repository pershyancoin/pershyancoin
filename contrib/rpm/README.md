...
The RPM spec file provided here is for Pershyancoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://pershyancoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Pershyancoin git repository](https://github.com/pershyancoin/pershyancoin) but are not part of
...
Currently only `Source10` is used, for the example `pershyancoin.conf` file.
...
in the pershyancoin git repository. At present only an SVG version of the Pershyancoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the pershyancoin
...
needed when 0.12.1 is released, a proper fix is already in the Pershyancoin git
master branch.
...
idea to build Pershyancoin Core against a static version of that library compiled
...
The `pershyancoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `pershyancoin-server` package. At this point, most RPM based
...
The `pershyancoin-server` package should have SELinux support. How to properly do
...
Pershyancoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/pershyancoin/). He has been
packaging Pershyancoin for Fedora at least since 2012.
...
4. I renamed the `pershyancoin` package that contains the Qt GUI to `pershyancoin-core` as
that appears to be how the general population refers to it, in contrast to
`pershyancoin-xt` or `pershyancoin-classic`. I wanted to make sure the general population
