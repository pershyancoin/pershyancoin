#!/bin/bash
# Copyright (c) 2013 The Pershyancoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Pershyancoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Pershyancoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Pershyancoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Pershyancoin 0.7 datadir."
    ;;
  3)
    echo "Detected Pershyancoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Pershyancoin 0.8 datadir."
    ;;
esac
